...
class BasicDataset(Dataset):
    def __init__(self, imgs_dir, masks_dir, type,scale=1):
        ...
        if self.type == 'MRI':
            for folder_name in os.listdir(self.imgs_dir):
                # print(folder_name)
                self.imgs_path += glob.glob(os.path.join(self.imgs_dir, folder_name, "*.jpg"))
        if self.type == 'CT':
            for folder_name in os.listdir(self.imgs_dir):
                # print(folder_name)
    ...
    def preprocess(cls,type, pil_img, scale):
        ...
        if type=='MRI':
            tf = transforms.Compose([transforms.Normalize([0.088], [0.149])])
        elif type=='CT':
            tf = transforms.Compose([transforms.Normalize([0.088], [0.149])])
...
SOURCE_IMG_DIR=r'D:\Project\Data\data\train_original\imgs'
SOURCE_LABEL_DIR=r'D:\Project\Data\data\train_original\labels'
SOURCE_VALI_IMG_DIR=r'D:\Project\Data\data\vali_big\imgs'
SOURCE_VALI_LABEL_DIR=r'D:\Project\Data\data\vali_big\labels'

TARGET_VALI_IMG_DIR=r'D:\Project\Data\data\naoshizhi_label\Intraparenchymal_hemorrhage_segmentation1\240\cases_images'
...
targetvalidationloader = DataLoader(dataset=targetvalidationset, batch_size=6)
